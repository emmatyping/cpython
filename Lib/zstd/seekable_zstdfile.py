...
class ZstdSeekableDecompressReader(_compression.DecompressReader):
    """A wrapper around ZstdDecompressor to match the Seekable format I/O"""
    ...
    def forward(self, offset):
        """Move forward and decompress the intermediate contents"""
        if offset is None:  # read until eof
            _data = self.readall()
        elif offset > 0:  # read until offset
            _data = self.read(offset)
        # otherwise, if offset is <=0, do nothing
...
class SeekableZstdFile(ZstdFile):
    ...
    _READER_CLASS = ZstdSeekableDecompressReader
    ...
    def __init__(
        ...
        *,
        ...
        if mode in ("r", "rb"):
            ...
            if max_frame_content_size != self.FRAME_MAX_D_SIZE:
                raise ValueError(
                    (
